...
class SecondNotificationService : Service() {
    ...
    private fun getNotificationBuilder(
        ...
        .setTicker("Third worker process is done, finishing up!")
        .setOngoing(true)
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        serviceHandler.post {
            countDown(notificationBuilder, seconds = 3)
            notifyCompletion(Id)
    ...
    private fun countDown(builder: NotificationCompat.Builder, seconds: Int) {
        ...
        for (i in seconds downTo 0) {
            Thread.sleep(1000L)
            builder.setContentText("$i seconds until completion").setSilent(true)
